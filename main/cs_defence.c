...
void setup_lake(struct tile map[MAP_ROWS][MAP_COLUMNS]) {
    ...
    if (lake_row == -1 && lake_col == -1 && lake_height == -1 
        && lake_width == -1) {
        printf("Error: Lake unwanted in game, ignoring...\n");
        return;
    }
    while (!valid_point(lake_row, lake_col) || !valid_point(lake_row 
        + lake_height - 1, lake_col + lake_width - 1)) {
        printf("Error: Lake out of bounds, try again...\n"
            "To skip lake creation input '-1 -1 -1 -1'\n"
            "Lake Position and Size (row col height width): ");
        scanf("%d %d %d %d", &lake_row, &lake_col, &lake_height, &lake_width);

        if (lake_row == -1 && lake_col == -1 && lake_height == -1 && lake_width == -1) {
            printf("Error: Lake unwanted in game, ignoring...\n");
            return;
        }
    }
...
void print_usage(void) {
    printf(
        ...
        "     ===============[     Command List     ]===============   \n"
        "  e                        \n"
        "    Enemy player spawns an enemy at the starting position.\n\n"
        "  t (row col)              \n"
        "    Defensive player places a new tower on the map.\n\n"
        "  m (no_steps)             \n"
        "    Enemy player moves all enemies towards the endpoint.\n\n"
        "  u (row col)              \n"
        "    Defensive player upgrades a tower to increase its effectiveness.\n\n"
        "  a (no_attacks)           \n"
        "    Defensive player attacks enemies within tower range.\n\n"
        "  r (row_spacing col_spacing row_offset col_offset)   \n"
        "    Enemy player can cause rain to alter the terrain and affect towers.\n\n"
        "  f (no_times)             \n"
        "    Both players can flood the terrain, affecting the path and towers.\n\n"
        "  c (row col dest_row dest_col)   \n"
        "    Enemy player creates a teleporter between two points on the map.\n\n"
        "  ?                        \n"
